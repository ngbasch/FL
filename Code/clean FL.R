...
#Define function ---------------------------------------------------
ReadDistribution<-function(month){
  #Notes:
  ...
  raw<-read_excel("Input/2020 Distribution Tracking sheet.xlsx", sheet = month,  col_names = FALSE, skip = skip_var)%>%
    clean_names()%>%
    mutate(r = 1:n(),
...
ReadDonor<- function(filename){
  print(filename)
  
  raw<-read_excel(paste0("Input/Robot Data/", filename))%>%
    select(Id:Donor)%>%
...
dist_raw<-lapply(ms,ReadDistribution)%>%bind_rows()%>%
...
donor_raw<-lapply(file_names, ReadDonor)%>%bind_rows()%>%
